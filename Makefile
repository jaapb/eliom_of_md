...
.PHONY: install install.byte install.byte install.opt install.static install.etc install.lib install.lib.byte install.lib.opt run.byte run.opt install.findlib
...
META: META.in
	sed -e 's#@@PKG_NAME@@#$(PROJECT_NAME)#g' \
		...
		-e 's#@@SERVER_ARCHIVES_BYTE@@#$(PROJECT_NAME).cma#g' \
		-e 's#@@SERVER_ARCHIVES_NATIVE@@#$(PROJECT_NAME).cmxa#g' \
		$< > $@
...
install.findlib: all META
	$(OCAMLFIND) install $(PROJECT_NAME) META
	mkdir -p `$(OCAMLFIND) query $(PROJECT_NAME)`/client
	mkdir -p `$(OCAMLFIND) query $(PROJECT_NAME)`/server
	cp $(CLIENT_CMI) `$(OCAMLFIND) query $(PROJECT_NAME)`/client
	cp $(CLIENT_CMO) `$(OCAMLFIND) query $(PROJECT_NAME)`/client
	cp $(SERVER_CMI) `$(OCAMLFIND) query $(PROJECT_NAME)`/server
	cp $(TEST_PREFIX)$(LIBDIR)/$(PROJECT_NAME).cm* `$(OCAMLFIND) query $(PROJECT_NAME)`/server

uninstall.findlib:
	rm -rf `$(OCAMLFIND) query $(PROJECT_NAME)`/client
	rm -rf `$(OCAMLFIND) query $(PROJECT_NAME)`/server
	$(OCAMLFIND) remove $(PROJECT_NAME)

reinstall.findlib: uninstall.findlib install.findlib
...
%.ml: %.mll
	$(OCAMLLEX) $< && ln -sf $@ ${LEXER_FILE}.eliom

%.ml: %.mly
	$(OCAMLYACC) $< && ln -sf $@ ${PARSER_FILE}.eliom
	cat Md_parser.mli_extended >> $(addprefix $@,i)
...
clean:
	...
	-rm -f *.type_mli
	-rm -f ${LEXER_FILE}.ml ${LEXER_FILE}.eliom
	-rm -f ${PARSER_FILE}.ml ${PARSER_FILE}.eliom
